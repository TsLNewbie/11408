...
## 57.导航区域
![](./images/1710834631515.png)
区域的运用：
![](./images/1710834680713.png)
道路上并非都是平路，会存在不同的路，而导航可以计算这些区域，进行更好的导航移动，以达到你想要的真实效果。

那怎么样去设置地图区域？
假设这个物体是水。
![](./images/1710835046196.png)

![](./images/1710835094266.png)

接下来进行bake，就会有这样的效果：
![](./images/1710835129544.png)

接下来就会这样了：
![](./images/1710835152769.png)
...
这个比例是**游戏窗口比例**。
![enter description here](./images/1710840401114.png)
![](./images/1710840493030.png)

- Canvas
	- 渲染模式很重要。
		- 覆盖：UI是覆盖在游戏场景之上的。
		- 摄像机：就当作是一个平面了。但是Canvas是永远面向摄像机的。（摄像机渲染）
		- 世界空间：也是摄像机渲染的，但是可以旋转了！（真的是一个平面了）
	- 排序次序：渲染优先级
- Canvas Scaler （缩放）
	- 可以调成屏幕大小缩放(这个更好，更多人用)  Scalse with Screen Size
- Graphic Raycaster （射线有关的）

覆盖：
![](./images/1710840772948.png)
摄像机
![](./images/1710840846508.png)

## 59.UI基础，锚点和轴心点

Image组件：
![](./images/1710841447450.png)
![](./images/1710841476888.png)

- Raycast Target(光线投射目标)
	- 接不接受射线 

中间这个是锚点：（锚点是附在父物体上的）
![](./images/1710841670589.png)

这个组件中的位置就是根据锚点的。
![](./images/1710841946196.png)
>注意：Width 和 Height 如果固定下来了，就会导致小屏幕看上去很大，大屏幕看上去很小。
这下就需要适配了。怎么做？：锚点

![](./images/1710842472496.png)
![](./images/1710842491595.png)

之后图片位置就不一样了，是相对于四个边的位置。
![](./images/1710842816207.png)

>这个有什么好处？当你锚点起来了，锚点是会被Canva背景改变。：画面拉伸了，锚点也会有所变化。
>平时的话绘把锚点直接与屏幕大小一致。

如果把锚点这样放：
![](./images/1710843029543.png)
那么图片不受Y轴影响了，如果你屏幕变高了，这个图片不会产生变化，但如果你屏幕拉长了，这个图片就会拉伸。
另外一边同理。

锚点的基本设置：
![](./images/1710843185450.png)
### 轴心点
中间位置就是轴心点
![](./images/1710843216374.png)
任何计算都是靠**轴心点**进行计算的。

如果把轴心点设置到左下角就会这样：
![](./images/1710843303774.png)

> 所以Rect Transform组件中的位置X Y，其实是 轴心点 相对于 锚点 的位置。


## 60.游戏中的文字，文本的使用。
![](./images/1710852276293.png)
![](./images/1710852431211.png)
Text里可以加**HTML来调整~**
...
导航中有：Explicit(属于是自己调整)
![](./images/1710853712660.png)


按钮如何去触发事件？就是这个东西：
![](./images/1710853809100.png)
...
把Content里加上Image物体，就可以做滚动视图了。
![](./images/1710857290447.png)

## 66.界面内容太乱了！面板的使用
假设如何去把界面（血条，头像）固定在左上角？我们可以这样做：
![](./images/1710857679237.png)

做法很正确，但是很麻烦！每个都得搞锚点都会很麻烦。

这就需要面板了：创建UI->Panel 面板

其实面板就是一个图片，但是单独搞一个图像叫做面板。
![](./images/1710857789277.png)

接下来就只管这个Panel物体即可。(物体都变成Panel的子物体了)
![](./images/1710857852443.png)

![](./images/1710857977720.png)
>设置完毕，取消显示Image组件即可。这样背景就没颜色了。


## 67.布局麻烦？常用的UI组件来帮你

假设父物体（白板)和子物体(箭头）关系是这样的：
![](./images/1710858270444.png)

无添加Mask 组件：
![](./images/1710858290978.png)

父物体添加Mask组件：
![](./images/1710858309037.png)



### 内容适应器

假设这个文本有多字，我希望这个框随着游戏过程改变这个框也发生改变。
![](./images/1710858384480.png)

组件：Content Size Fitter
![](./images/1710858463797.png)
...
>15. 项目部署
>16. 综合案例：角色扮演游戏
当然还有进阶内容：关注公众号GamerFeiYu，可以获取更好，更进阶的组件使用！
