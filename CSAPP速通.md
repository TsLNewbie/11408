...
![](./images/1717150923119.png)

## 7.6 重定位

Relocation(重定位) : 链接器合并输入模块，并为每个符号分配运行时地址。
...
![](./images/1717151126133.png)
第一步：把所有相同类型的section 合成为一个

![](./images/1717151385686.png)
在64位linux系统中，ELF可执行文件默认从地址 0x400000处开始。
ELF文件中 header 和 text section之前还有一些信息
因此假定从 0x4004d0 开始

第二步：重定位section的符号引用

![](./images/1717151507134.png)
callq 就是函数sum 的调用，此时call指令的目的地址是0，显然不是函数sum真正的地址。

链接器需要修改对符号sum的引用，使它指向正确的运行地址。
需要依赖于 **可重定位条目** 的 数据结构

### 重定位条目

>可重定位目标文件 是由汇编器产生的。
>当汇编器在生成这个文件时 ，并不知道数据和代码最终放在内存的哪个位置。
>汇编器也不知道该模块所引用的外部定义的函数以及全局变量的位置。
>
>所以，当汇编器遇到最终位置不确定的符号引用时，就会产生 一个 **重定位条目**（相信后人的智慧）

>可重定位条目功能： 告诉链接器在合成可执行文件时应该如何修改这个引用。

关于代码的 重定位条目 放在.rel.text中
关于已初始化数据的 重定位条目 放在 .rel.data中

ELF 重定位条目的结构体定义：

![](./images/1717162989419.png)

offset = 被修改的引用的节（section）偏移量
type = 链接器会根据这个 来修改新的引用
symbol = 被修改的引用 是 哪一个符号
addend = 符号常数
...
![](./images/1717174982615.png)
一个是对sum 的引用。

另一个是对全局变量array的引用。

看看链接器是如何使用 重定位条目 来进行 重定位 的

### 相对地址引用 
例如：指令call的起始地址 位于 **字节偏移 0xe** 的地方
0xe8 = 指令call的操作码。

![enter description here](./images/1717175137388.png)
在重定位之前，紧跟在操作码之后的内容被 汇编器 填充为0
这一部分内容就是链接器要根据重定位条目来确定。

对于函数sum的 重定位条目 由4个字段组成，具体如图：

![](./images/1717175349004.png)
首先，链接器需要根据 重定位条目 计算出引用的运行时地址。 
...
![](./images/1717211253468.png)

实际上这一步 = 求两个地址之间的相对位置。

![](./images/1717211389562.png)
接下来执行到callq的时候，PC值=  4004e3，执行call指令需要把PC值压入栈，然后跳转（这个就是前几章的知识）


>运行的时候当前PC值要加多少，就刚好是call的位置
>加addend的原因是，在执行跳转后，pc(%rip)就已经跳到下一个指令了，需要算术调节回来。

### 绝对地址引用

![](./images/1717211500641.png)
计算公式：

![](./images/1717211540762.png)

![](./images/1717211568396.png)


由于是小端法，要翻回来。

最后形成的 目标文件和text section 和 data section的内容。

![](./images/1717211595848.png)

## 7.7 可执行目标文件

Executable Object Files(可执行文件)

ELF可执行文件格式 和 可重定位目标文件的格式

![](./images/1717211796695.png)
可执行文件也包含 ELF header，描述了文件的整体格式。
其中有一项是程序的入口，也就是程序运行时要执行的第一条指令的地址。

.init 定义了_init函数
程序的初始化代码会调用这个。（对函数进行初始化）

.text rdata data 都和重定位一样，不同的是已经重定位到最终的运行时内存地址上。

代码段和数据段需要被加载到内存执行。

符号表和调试信息 就不会加载到内存。

![](./images/1717218658460.png)
看看可执行程序prog的程序头部表，也就是segment 头部表
：描述了**代码段**，**数据段**与内**存的映射关系**。

![](./images/1717218738399.png)

![](./images/1717218815452.png)

这个flag标志告诉我们这个段只有读和执行的权限

![](./images/1717218878208.png)这个就只有读和写权限

> flag，r = 可读 w = 可写 x = 可执行 

![](./images/1717219004994.png)
表示这个segment 在可执行文件的偏移量。
由于代码段位于可执行文件的开始处，off = 0

![](./images/1717219055705.png)
这个表示这个segment 起始内存地址 在 0x400000

![](./images/1717219129159.png)
code segment（代码段）大小 0x69c 字节， 所以内存中也是占 0x69c个字节。
包括：ELF header, 程序头部表, .init , .text , .rodata section

以上是代码段与内存的映射关系。

接下来是数据段和内存的映射关系。

![](./images/1717219280945.png)
起始地址： 0x600df8

文件大小：0x228

占内存大小： 0x230 （多了 8byte) (16进制！！)

这多出来的  8byte 用来存放 .bss section 的数据。

虽然.bss section 不占用 可执行文件的空间 ，但是.bss中的数据在运行时需要被初始化 = 0



对于任何一个段，链接器必须选择一个起始地址vaddr

使得：

![](./images/1717219555846.png)

off = segment 在 可执行文件中的相对于起始位置的 偏移量（offset)
align = 程序头部表中 指定的 对齐量。
mod = 取模

这个过程是优化的一种：使得执行时，可执行文件中的段能够高效的传送到内存中。

### 执行 可执行文件

调用加载器。

![](./images/1717220410079.png)


加载器 将 可执行目标文件中的代码和数据从磁盘复制到内存中。
然后跳转到程序的入口来运行该程序。
这个将程序从磁盘复制到内存并运行的过程 叫做 **加载**（名词）

每个linux程序都有一个运行时内存镜像，如图：

![](./images/1717220431825.png)
从下往上，
代码段，数据段，堆，共享模块保留的，栈，操作系统的代码和数据保留（Kernel Memory）

其中：
堆的增长方向是从 **低地址到高地址**

用户栈的起始地址：2^(48) -1 ，这里是最大的合法用户地址。

栈的增长方向是从 **高地址到低地址**

Kernel Memory 是从 地址 2^(48) 开始。（对用户代码是不可见的）

代码段，数据段 和 堆之间是相邻的。
同时 栈 的起始位置 放在了 最大的合法用户地址处。

数据段 有地址对齐的要求， 所以 代码段和数据段 之间是有间隙的。

防止攻击， 分配栈、共享库、堆的运行时地址 ，会用上 **地址空间随机化**（名词）

不过相对位置是不变的。

当加载器运行时，他为程序创造图中所示的内存镜像
根据程序头部表的内容，加载器将 可执行文件的section 复制到内存相应的位置。
接下来加载器跳转到入口处，start地址。

libc_start_main() 初始化执行环境

![](./images/1717221056961.png)
（每一行运行完才到下一行)
...
- 静态库需要定期维护和更新
- 几乎每个C程序都要使用标准的I/O 函数
	- 在运行时，每个进程都需要复制一次库数据。如果是成千上万的进程，那就复制很多很多了。对内存资源来说是一种极大的浪费。

### 共享库

>**共享库(Shared Libraries)** 是一种特殊的可重定位目标文件，在linux通常用.so表示。
>Windows也有，.dll 表示（dll == Dynamic Link Library == 动态链接库）
>
>共享库在运行或加载时，可以被加载到任意的内存地址。
>和在内存中的程序链接起来，这个过程称为动态链接。

构造共享库：

![](./images/1717222721580.png)
其中： 
-shared 指示编译器创建一个共享的目标文件 
-fpic 选项，告诉编译器生成位置无关的代码
上段执行，就可以获得 libvector.so 的共享库

下段执行，就可以生成一个可执行文件prog2


> libvector.so 中的代码和数据 并没有真的被复制到可执行文件prog2中
> 只复制了符号表和一些重定位信息。

在可执行程序prog2被加载运行时，
加载器会发现可执行程序prog2中存在一个名为：.interp的section
这个section中包含了 **动态链接器**的**路径名**
这个动态链接器本身也是 == 共享目标文件(ld-linux.so)

接下来，加载器会将这个 动态链接器 加载到内存中运行。
然后由 动态链接器 执行 重定位代码和数据的工作。

![](./images/1717224971109.png)
具体过程:

- 首先是将libc.so的代码和数据重定位到某个内存段
- 然后是重定位libvector.so中的代码和数据到另一个内存段
- 接下来重定位prog2中由libc.so和libvector..so定义的符号引用
- 动态链接器把控制权交给应用程序prog2
	- 这个时刻开始，共享库的位置就固定了，执行过程都不会改变。


更多情况:
应用程序还可能在它运行时要求动态链接器 加载和链接**某个共享库**,而无需在编译时将那些库链接到应用中.
...
- 构建高性能 Web 服务器
	-  生成动态内容
	-  例如：生成个性化web 或者账户余额。
	-  思路：将每个生成动态内容的函数打包在共享库中，当一个来着browser的请求到达时，Web Server 动态的加载和Link 适当的函数，然后直接调用它。由于函数会一直缓存在Server 的地址空间中，所以只需要一**个函数**调用就可以处理随后的请求。
	-  要更新内容，不需要停止服务器
...
函数dlopen = 动态加载共享库，
RTLD_LAZY 指示链接器将 符号解析的操作推迟。一直推测到共享库中的代码执行时再符号解析


函数dlsym = 选用其中一个函数

handle = 函数dlopen已经打开的共享库的句柄
addvec = 符号名 。（符号存在，返回符号地址，否则返回NULL)

函数dlclose 来卸载这个共享库。

![](./images/1717225900526.png)

# 第八章 异常控制流

## 8.1异常控制流
从CPU开始上电运行，到断电，假设 程序计数器的值是如下序列。

![](./images/1717227304657.png)

平滑： （程序）Ik ， Ik+1 的内存位置是**相邻**
突变：跳转，函数调用，返回导致的两次程序内存位置**不相邻**。

系统需要对系统状态的变化做出反应：
例如：从网络中传输的数据包 到达 网络适配器后，需要将数据放到内存中。接下来CPU要处理这种情况。

>这种突变 称为 **异常控制流**。

理解异常控制流重要性：
- 帮助理解重要的系统概念
	- 异常控制流 是操作系统 用来实现 I/O ,进程以及虚拟内存的基本机制。
- 帮助理解应用程序是如何与操作系统交互的
	- 磁盘写入数据，创建一个进程，应用程序通过系统调用做的。
- 帮助编写一些有趣的应用程序
	- Unix shell 和 web服务器的应用
- 理解理解并发
	- 异常控制流 是并发的基本机制
- 软件异常是如何工作的
	-  C++ / Java 提供了 try-catch-throw 捕获软件异常
	- 软件异常允许程序进行 非本地跳转 来响应错误。
		- 非本地跳转（名词）：违反 通常的调用/返回栈规则 的 跳转
		- 通过函数setjmp 和 longjmp


### 异常 Exception

当处理器正在执行应用程序中某一条指令时，系统发生了一次事件，这个事件可能和当前指令的执行**直接相关**，也可能**无关**。
：例如当前指令执行的是除以0的操作，那么此时异常的发生就与当前指令有关
：如果事件是一个I/O请求的完成，那就与当前指令的执行无关

在任何情况下，当处理器检测到有事件发生时
处理器从执行 应用程序 切换到 **异常处理程序。**
当异常处理程序完成后，根据引起异常的**事件类型**选择**是否返回。**

![](./images/1717227864477.png)
这是因为处理异常需要**硬件和软件紧密配合**。

### 异常编号

> 系统中为每种类型的异常都分配了**唯一**的异常编号

其中有些编号是 CPU 的设计者分配的：被零除，缺页，算术运算溢出。
其他号码是 操作系统内核 的设计者分配的：系统调用、 外部I/O设备的信号

CPU感应到异常，会根据 异常编号 从异常表 找出对应的 异常处理程序 来处理异常。

![](./images/1717228202471.png)

>异常表是在系统启动时，操作系统分配和初始化的一个跳转表。
>异常号 就是这个表的索引号。

异常表的起始地址是保存在CPU中的一个特殊的寄存器中。
这样一来，通过 **异常表基址寄存器** 与异常号可以确定对应的异常表项
异常表项中的内容是对应的异常处理程序的起始地址


>异常的处理 类似于 执行一个间接的函数调用
...
### 异常 vs 函数调用
- 返回**当前指令** 或者 **下一条指令**
	- 函数调用：在跳转到目的函数之前，CPU首先将返回地址压入栈中
- CPU的**状态**压入栈中
- 控制从 用户态（User） 转向 内核态（Kernal）
	- 这些内容压到 **内核栈** 中 ，并非 用户栈
- 运行在**内核态**
	- 对所有的系统资源都有 **访问权限**。
...
- 异常处理程序将 控制 返回到 **当前指令**
	- CPU继续执行 当前正在执行的指令
- 异常处理程序将 控制 返回到 **下一条执行**
	- CPU即将会执行的 下一条指令
-  异常处理程序 **终止**程序

## 8.2 异常

>CSAPP 原书中 异常分成四类： 中断(Interrupt)、陷阱(Trap)、故障(Fault)、终止(Abort)
>中断是**异步**的。
>这个**异步**的含义是中断是由处理器外部的I/O设备产生的
>其他三种是**同步**的，
>是CPU执行当前指令产生的结果。

>这里异步和同步的主要区别是异常产生的原因来自 CPU外部 还是 内部


### 中断的具体处理过程

![](./images/1717228894343.png)
键盘是 I/O设备，当我们敲一下键盘，
键盘控制器 会向CPU的 **中断引脚** 发送信号来触发 中断
同时会将 异常号 放到 系统总线 上，这个 异常号 标识了引起 中断 的 **设备**

CPU在执行完当前指令后，发现**中断引脚**的电压变高了
于是从系统总线上读取异常号，判断是哪个设备发起的中断
然后调用相应的中断处理程序来处理中断
中断处理完毕后，CPU返回继续执行下一条指令
（下一条指令指的是如果没有发生中断，之前控制流中正常执行的指令）

好像中断没有发生过一样。~

### 陷阱Trap

>陷阱是一种 **故意触发**的异常 ，它是执行一条指令的结果
>最重要的用途：用户程序 和 操作系统内核 之间提供一个类似 函数的接口
>
>**陷阱 =》系统调用**

![](./images/1717229181585.png)


当应用程序需要读取文件或者创建新的进程时
此时需要向内核请求服务，处理器提供了一条特殊的指令一一syscall
执行syscall指令会导致一个 陷阱
陷阱处理程序 解析参数，并调用适当的 内核程序 提供系统级的服务
执行完毕后，返回到指令syscall之后的指令继续执行


### 故障 Faults

> 故障是由**错误情况**引起的，不过故障是有可能被 故障处理程序 修复的

![](./images/1717229287617.png)
假设当前的指令导致了 故障 发生，处理器会将 **控制转移** 给 **故障处理程序**
接下来故障处理程序运行，如果**能够修正**这个错误的情况
它就将 控制 返回到**引起故障的指令** ，然后 **重新执行**引发故障的这条指令
如果处理程序**无法修正**这个错误，就会**终止**引起故障的应用程序

一个经典的故障：**缺页异常**。（名词）

假如当前指令中引用了一个虚拟地址
不过与该地址对应的 物理页面不在内存中 ，这个时候需要从 磁盘读取数据到内存
此时当前指令就会引发**故障**
之后 缺页处理程序 会从磁盘加载对应的页面到内存
然后将控制返回给**引起故障的指令**
>**注意：** 此时继续执行的指令是引起故障的指令

当这条指令再次执行时，相应的物理页面已经在内存里了
就不会引发故障。


### 终止Aborts

>终止是由**不可恢复的致命错误**导致的，通常是一些硬件错误

例如DRAM或者SRAM的存储位被损坏时，会导致奇偶校验出错。

![](./images/1717229504005.png)
不返回，直接终止。

### x86-64系统的异常

在x86-64系统中一共定义了256种异常类型,
其中0~31号异常是由Intel的架构师定义的，因此对于任意的x86-64系统都是一样的.
编号32~255所对应的异常是由操作系统定义

![](./images/1717229576426.png)
#### CPU架构师定义的异常

异常号 = 0 ：除法故障。 报告为："浮点异常"
当一个指令试图进行除以零的操作时，就会发生除法错误
此时 异常处理程序 不会试图恢复这个错误，而是选择 **终止程序**

异常号 = 13：，一般保护故障 。“段错误”
程序引用了一个未定义的虚拟内存区域导致的。
程序试图去写一个只读的文本段。
系统不会尝试去修复。

异常号 = 18 ：机器检查。
该异常是硬件发生错误时导致的。
机器检查处理程序会**终止**引发异常的应用程序。

#### 操作系统定义的异常

Linux提供了几百种系统调用。

![](./images/1717229832344.png)
每个系统调用都有一个**唯一的**整数号
这个整数对应的是 内核中 跳转表 的**偏移量**

>注意：跳转表 != 异常表

### 代码示例

![](./images/1717229902309.png)
第一章用的是 printf，而这次我们用**系统级函数write** 来实现 相同功能。

其中：
第一个参数1表示将输出发送到标准输出stdout
第二个参数是要输出的具体内容，
第三参数13表示输出字符串的长度

系统调用是通过陷阱指令syscall:来实现的,
如何使用汇编代码编写一个程序来实现系统级函数write的调用.

![](./images/1717230140456.png)

其中 9 ~ 13 行 是调用函数write.
Linux系统调用的参数都是通过寄存器来传递的.
寄存器rax用来传递系统调用号.
...
>Processes（进程） = 一个正在执行的程序实例


当我们在shell 执行 可执行文件
shell创建一个新的进程，然后处理器运行这个可执行程序

主要关心的是进程提供给 **应用程序** 的 **抽象**!!!
运行一个程序时，我们会得到**假象**。
...
用调试器来控制程序单步执行，我们会看到一系列的 程序计数器(PC) 的数值。
这些数值与 可执行程序 中的指令是一一对应的。
这个PC值的序列叫做逻辑控制流，简称**逻辑流**(名词)


假设我们的系统中运行着三个进程，分别是进程A,进程B以及进程C
对于一个单核处理器，它在某一个时间段内只能执行一个进程.
处理器的物理控制流被分成了三个逻辑流，每个进程一个。

每个竖直的线表示一个进程的逻辑控制流的的一部分

![](./images/1717255112479.png)
> 一个逻辑流的执行 在时间上与另一个流 **重叠** 的情况，称为**并发流**
> 这两个流的执行被称为 **并发运行**

例如：进程A 和 进程B ， 进程A 和 进程C 是**并发运行**的。
不过 进程B 和 进程C 并不是并发运行的。

因为 进程C 开始运行之前 进程B **执行结束**了，所以二者不能称为并发。

>**并发**和**并行**会混淆。
>并行 = 两个进程在**不同**的CPU核上**同时运行**
>
>并发是**交替运行**的，而并行是**同时运行**的 （重点）

>计算机网络：时分复用


除此之外，进程也为每个程序提供了另外一种假象。

- 程序本身**独占**整个系统的地址空间。


![](./images/1717255352969.png)

地址空间的**低地址**部分是预留给**应用程序**的，包括代码段、数据段、堆和栈
代码段总是从地址0x400000处开始。

地址空间的**高地址**部分是预留给**操作系统内核**(Kernel Memory)的，属于用户代码不可见的内存区域。

在Linux系统的根目录下有一个/proc的文件夹，如图所示：
这些文件记录的是 内核相关的 **数据结构**

![](./images/1717255512403.png)
CPU信息：

![](./images/1717255559457.png)



### 用户模式 和 内核模式 User mode / Kernel mode
为了 **限制** 应用程序 执行某些特殊的指令以及 **限制** 可以访问的地址空间范围

通常处理器 通过 **控制寄存器** 的 **模式位** 来实现这些限制功能
该寄存器描述了进程当前的权限

当设置了控制寄存器的 **模式位** 后，进程就运行在**内核模式**（超级用户模式）

对于一个运行在 内核模式 的进程可以执行指令集中的任何指令
并且可以访问系统中任意的内存位置

无 设置模式位 = 用户模式 == 不允许执行特权指令（停止CPU, 改变模式位，发起I/O操作等）

用户模式的进程也不能 直接引用内核区域的代码和数据（试图干，触发**保护故障**，被终止运行）
不过用户程序可以通过 **系统调用** **间接的** 来访问内核的代码和数据

应用程序在一开始是运行在用户模式下
进程从用户模式——>内核模式 需要通过 **中断、故障或者系统调用** 的方式
异常发生，执行 异常处理程序 处理异常
此时，CPU的模式就会从 用户模式 变为 内核模式

当返回到应用程序继续执行时，会从内核模式改回到用户模式

![](./images/1717255877685.png)
### 上下文 Context
[[到底什么是上下文（Context）-CSDN博客](https://blog.csdn.net/caizir913/article/details/108826764)]
内核为每一个进程维持了一个上下文。
>上下文 —— 内容重新启动一个被抢占的进程所需的**状态** （名词）
>它是由一些对象的值组成。


![](./images/1717256112639.png)
包括：
- 通用目的寄存器
- 浮点寄存器
- 程序计数器
...
- 状态寄存器
- 内核栈
- 内核数据结构（包括：描述地址空间的页表，包含有关当前进程**信息** 的 进程表 以及包含进程 **已打开文件** 的 信息表）

>在进程执行的某些时刻，内核可以决定抢占当前进程。重新开始执行先前被抢占的进程。
——**进程调度。**（名词）
是由内核中的**调度器**来执行的

当内核选择一个新的进程运行时 —— 内核**调度**了这个进程

当内核 **调度** 了一个新的进程运行后，它就抢占当前进程
并使用一种称为 **上下文切换** 的机制来将控制转到新的进程
...
1. 保存 **当前进程**的上下文
2. 恢复某个先前**被抢占**进程的上下文
3. 将**控制** 传递给这个**新恢复**的**进程**

**中断** 也可能引发上下文切换

例子：（重点：这种看视频是最好的，挺好理解）

![](./images/1717256777089.png)

最开始 进程A 运行 在 **User Mode** 下

当进程A需要从 Disk（磁盘) 中读取数据时
它会执行 **系统调用** 函数read 陷入到内核中

然后 **陷阱处理程序** 向**磁盘控制器**发起一个**DMA的请求**
并且安排 磁盘控制器 完成从磁盘到内存的**数据传输**之后
磁盘需要向处理器发送 **中断**

磁盘读取数据需要一段较长的时间。（几十毫秒）
在CPU中，几十毫秒很漫长。而且这段时间空着也不好，所以就会运行 进程B
内核执行从 进程A 到 进程B 的**上下文切换** <————————

随后，进程B在用户模式下运行
直到磁盘发出一个**中断**的信号，表示数据已经从磁盘传送到了内存。
内核判定进程B已经运行了 足够长的时间

就执行一个从进程B到进程A的**上下文切换**

将控制返回给进程A中紧跟在系统调用read之后的那条指令
然后进程A继续执行，直到下一次异常发生

## 8.4 进程的创建

>主要来看一下创建进程时用到的 **系统函数** 一一**fork**

在shell中运行hello程序时
把shell看成 **父进程** ，可执行程序hello看成 **子进程**

**父进程** 通过 调用fork函数 来创建一个新的可以运行的 **子进程**

>**程序和进程的区别**
>程序： 一段**代码和数据**。
>例如:Hello程序在执行之前是以**文件**的形式存储在 磁盘 上
>在执行的过程中，程序是以 **段**(Segment) 的形式存在于内存的地址空间中
>
>进程：正在执行中程序的一个具体的**实例**
...
- 运行
	- 进程要么在CPU上执行，要么等待被执行(进程最终会被 **内核调度** 执行的)
- 暂停
	- 进程被挂起，它不会被调度执行。
	- 收到信号，就会进入暂停状态,并且一直保持： SIGSTOP SIGTSTP SIGTTIN SIGTTOU
	- 收到信号，进程再次开始运行： SIGCONT (Continue)
	- 关于信号是一个**软件中断**的形式。
- 终止
	- 进程**永远不会**再运行了。
	- 原因有三：
...
>这与操作系统中介绍进程的状态有些差别。
>这是因为以上关于进程状态的分类是从**程序员的角度**来看的。


### 如何创建进程

![](./images/1717257698702.png)


返回值的类型 pid_t 在文件 types.h 中被定义为 int类型，通常调用一个函数只会返回一次。

>然而函数fork被调用一次，却会**返回两次**
一次是返回到**父进程**，另外一次是返回到新创建的**子进程**。


代码示例：

![](./images/1717257794123.png)
调用了fork函数，返回值存放在pid中。

父进程中，函数fork的返回值的是**子进程的PID**
在子进程中，函数fork的返回值的是**0**

由于子进程的进程号**总是大于0**的
可以利用返回值的不同来区分

程序究竟实在父进程中执行，还是在子进程中执行

运行图中程序，得到图下的结果：

![](./images/1717258138535.png)
使用进程图的方式：程序的具体执行过程。

![](./images/1717258335312.png)

程序最开始时，父进程将变量x设置为1

当父进程调用函数fork时，父进程创建一个子进程
由于函数fork返回到 父进程 与 子进程 的返回值不同
根据程序的逻辑，二者执行的代码也会发生变化

在子进程中，fork返回0
因此，对于子进程来说将会执行if内的printf,打印x=2(\+\+x)之后退出

对于父进程中来说，fok返回子进程的进程号
因此执行这一条printf语句，打印x=0(--x)之后退出


### 父进程 vs 子进程
- 调用一次，返回两次
- 独立**并发**（Concurrent,交替）执行
	- 父进程 先执行printf打印，然后是子进程执行（不同系统可能不同）
-  相同但是独立的地址空间
	-  当父进程调用fork函数创建子进程后，如果我们**立刻暂停**程序继续执行，也就是说不等到函数fork 返回到 父进程和子进程 之前就执行一个**暂停**的操作。此时父进程与子进程的地址空间中的内容是相同的，二者具有相同的用户栈、本地变量值、堆、全局变量值、代码。
	-  然而父进程和子进程是独立的进程，他们都有自己的私有地址空间，后面父进程和子进程对变量x所做的任何改变都是**独立的**。（子进程复制了父进程所有内容）
- 共享文件  
	- 子进程继承了父进程所有打开的文件
...
![](./images/1717259023248.png)
注意：第二次fork，父进程和子进程都会执行。

最后有 4次hello的输出，因为有4个进程执行了。
由于四个进程是**并发执行**，所有4个hello输出是任意顺序。

## 8.5 函数execve 与 函数waitpid

实际上程序在加载以及运行的过程中，除了需要使用fork函数之外，还需要**函数execve**。

![](./images/1717307548484.png)

与fork函数不同的是：函数execve调用之后，**不会返回**。
...
![](./images/1717307618252.png)

ls —— 应用程序的名字
-lt 和路径 —— 可执行程序 ls 的参数列表

#### 参数列表argv：
数据结构图：

![](./images/1717308054754.png)


**指针argv** 指向一个指针数组
这个指针数组中的 **每一个**元素都 **指向** 一个字符串
这个字符串就对应我们输入的参数
...
![](./images/1717308347754.png)

envp也是指向一个指针数组
每一个指针指向一个环境变量的字符串
环境变量包括：当前的工作目录PWD、用户名等信息

番外：通过这个可以知道计算机中的环境变量。

![](./images/1717308137753.png)
>个人经历：
>当初做python的时候，需要下载python3.1，还有把python加载到环境变量中，才能运行python的代码，那么从这个就有关系了。

[环境变量是什么？有什么用？_环境变量是什么意思-CSDN博客](https://blog.csdn.net/Mr_ZhouR/article/details/81776470)


>总结，**参数列表argv** 和 **环境变量列表envp** 格式是一样的，但是指向的变量不一样。

### 函数 execve 的作用

函数execvel的作用就是 **调用加载器** ，在执行可执行程序的main函数之前
启动代码需要设置 **用户栈**，并将控制传递给新程序的**主函数**
该主函数如图：

![](./images/1717308746569.png)
三个参数：
argc —— argv中非空指针的数量
argv —— 指向数组argv中的第一个元素
envp —— 指向数组envp的第一个元素

用户栈的组织结构：

![](./images/1717308855042.png)
其中包含 参数列表 以及 环境变量字符串 ，

栈顶是系统启动函数libc_start_main的栈帧
由于main函数是libc_start_main调用的
所以libc_start_main的栈帧在 **高地址** 处，main函数的栈帧在 **低地址** 处


### 僵死进程（zombie）

>当一个进程由于某种原因**终止**时，内核并不是立即把它从系统中清除
>此时进程被保持在一种已终止的状态中，直到被它的父进程回收
>把一个终止运行但是还未被回收的进程称为**僵死进程**(名词)


一个僵死进程虽然没有在运行，但是仍引旧在消耗系统的内存资源。

#### 函数waitpid

在Linux系统中父进程可以通过 **函数waitpid** （重点）来等待它的 子进程终止或者停止

![](./images/1717309518970.png)
稍微复杂：
- pid > 0
	-  等待的进程是一个单独的子进程，子进程的ID = pid的值
- pid = -1 
	- 等待的进程 是 由父进程创建的 所有子进程组成的**集合**
- statusp (非空) 
	- 函数会在status中放上导致返回的子进程的**状态信息**
	- 在wait.h 的头文件 定义了解释 status 参数的几个宏
		- WIFEXITED(status) —— 通过exit / return正常终止 = true
		- WIFSIGNALED(status) —— 因为一个未捕获的信号终止 = true
		- WEXITSTATUS(status) 
		- WIERMSIG(status)
		- WIFSTOPPED(status)

>为什么使用waitpid结束，因为有的时候父进程 可能要获得 子进程的某些信息


#### 代码示例：

![](./images/1717310117468.png)
父进程通过循环创建了N个子进程
可以确定每个子进程都会执行函数exit
每个子进程会以唯一的退出状态退出。

接下来，父进程用函数waitpid作为while循环的测试条件
等待它所有的子进程终止

可以看到函数waitpid有三个参数
参数-1，表示父进程所等待的进程**集合**都是该父进程创建的子进程

接下来检查子进程的退出状态
如果子进程是正常终止的，也就是说子进程是调用函数exit终止的

根据前面所讲到的，这个宏WIFEXIED的返回值就是true
那么父进程就会将这个子进程的信息打印出来 （代码内容 printf)

>需要注意的是，程序不会按照**特定的顺序** **回收**子进程

也就是说当我们再次运行该程序时，两个子进程都可能以**相反的**顺序被**回收**


为了消除这种不确定，进行代码改进。

![](./images/1717310526981.png)
改进后就代码可以按照 父进程创建子进程 的**相同顺序**来回收这些子进程

父进程用一个**数组pid[N]** 来存储所有子进程的PID
通过PID的值作为第一个参数来调用函数waitpid

这样一来，父进程就可以按照创建子进程的顺序来等待每个子进程的结束

同样，根据宏WIFEXIED的返回值来判断子进程是否正常退出。
解释结果是true,打印子进程的相关信息，否则输出非正常终止的信息。

## 8.6 信号

> 更高层次的软件形式的**异常**一一**Linux信号**
> 它允许内核和进程中断其他的进程。

部分Linux系统上所支持的信号：

![](./images/1717310664915.png)
正常情况下，底层硬件的异常是由 内核异常处理程序 处理的。（用户不可见）
信号提供了一种机制，用来通知进程发生了哪些异常情况。

当一个进程试图执行除以0的操作：
那么内核会发送给该进程一个**SIGFPE的信号**，这个信号对应的是**浮点异常**的事件。

如果进程执行非法指令：
那么内核会发送给该进程一个**SIGILL的信号**，该信号对应的事件是**非法指令**。

一个进程正在Linux shell中运行，此时我们按下Ctrl+C键：
那么内核就会发送一个**中断信号SIGINT**给当前的进程，终止它的运行。

### 进程组 Process Groups 

每个进程都只属于一个**进程组**，每个进程组都有自己的ID值来唯一标识，这个ID是一个正整数

可以通过**函数getpgrp**来获取当前进程所属的进程组ID值。

![](./images/1717311220281.png)

>这个部分出错了，并非setpgrp 应该是 setpgid
>[Linux系统编程——进程控制（三）_linux setpgid-CSDN博客](https://blog.csdn.net/qq_17808707/article/details/88180244)

默认情况下，一个子进程和它的父进程属于一个进程组。
不过进程可以通过**函数setpgrp**来改变自己或者其他进程的进程组。
其中：
pid —— 进程原来的进程组的ID
pgid —— 更改后的进程组的ID

pid = 0,那么就使用**当前进程**的PID值
pgid = 0,那么就用pid指定的进程PID作为进程组的ID值

例子：
如果在**进程号为15213**的进程中调用了函数setpgid(0,0),其中pid = pgid = 0
那么这个函数执行完毕后，就会创建一个新的进程组 (ID值 = 15213)

然后把进程15213加入到这个新的进程组中
执行完毕后，进程组的ID值和进程的pid都是15213
...
>通过/bin目录中的kill程序可以向其他的进程 发送**任意的信号**

![](./images/1717312154290.png)
第一个指令：（进程）
向进程号为15213的**进程**发送信号9
信号 9 == 杀死进程。（该进程终止运行）

第二个指令：（进程组）
-15213表示向**进程组**15213发送信号
这个信号9将会发送到到该进程组中的每一个进程中
这样一来，该进程组中的所有进程都将终止运行。

注意，这里我们用了程序kill的完整路径，是因为有些shell中内置了命令kill。


#### 从键盘发送信号

![](./images/1717312346271.png)

当我们在键盘上输入Ctrl+C键会 发送一个**中断信号**到**前台进程组**中的**所有进程**中

**前台进程组：**（名词）

当我们在shell中输入图中的命令时，
Shell会创建一个前台作业，这个前台作业是由两个进程组成的
其中程序ls可以看成一个进程，程序sort可以看成另外一个进程

这两个进程是通过管道连接起来的，程序ls的输出结果将会作为程序sort的输入

Shell为每个作业创建一个独立的进程组，如图（一个前台，两个后台）

![](./images/1717312389825.png)
前台作业中的父进程PID为20，进程组（pgid）的ID也为20
创建的两个子进程的PID分别为21和22，这两个进程的进程组ID都是20

在任何时刻，最多有一个前台作业和0个或者多个后台作业
当键盘上输入Ctl+C时，默认是终止前台作业
类似的，输入Ctl+Z会挂起前台作业

#### 函数kill 发送信号

![](./images/1717312540073.png)
pid > 0 ：
函数kill 发送信号sig 给进程pid

pid = 0 ：
函数kill 发送信号sig 给 调用进程所在进程组中的**所有进程**（包括调用进程自己）

pid < 0 ：
函数kill发送信号sig给 **进程组pid**中的**每个进程** （进程组ID = |pid|）

#### 使用函数alarm 发送信号

进程可以使用函数alarm向它自己发送信号(SIGALRM)
其中 **参数secs** 表示函数alarm安排内核在secs秒后发送一个SIGALRM信号给调用进程
secs = 0：
就不会调度新的闹钟了。

### 接收信号

当内核把进程P从内核模式切换到用户模式时
此时会检查进程P的 **未阻塞的待处理** 的**信号集合**

如果这个集合为空，那么内核将 **控制传递** 到进程P的逻辑控制流中的下一条指令
如果集合是非空的，那么内核选择集合中一个**信号k**,**强制进程p接受信号k**

接收信号会触发**控制转移**到信号处理程序
在信号处理程序完成处理之后，它将**控制返回**给**被中断的程序**

每一个信号类型都有一个预定义的默认行为：
- 进程终止
	- 收到信号SIGKILL后，终止运行
- 进程终止并转储内存（dumps core）
	- 把代码和数据的内存镜像写到磁盘上。
- 进程挂起 直到被 SIGCONT 信号重启
-  进程忽略该信号
	-  信号SIGCHLD 可忽略。

### 信号情况

>信号接收以及处理的方式与异常的处理有许多相似之处

![](./images/1717313004459.png)
一个发出而没有被接受的信号叫做**待处理信号**（名词）
在任何时刻，一种类型的信号最多只会有一个待处理信号

也就是说如果一个进程有一个类型为k的待处理信号
那么任何接下来发送到这个进程的类型为k的信号都不会排队等待

它们只是被简单的丢弃
所以一个待处理信号最多只能被接受一次

>需要特别注意的是信号处理程序可以被其他信号处理程序中断


例子：

![](./images/1717313146025.png)
...
9.3 虚拟内存作为缓存的工具
9.4 地址翻译
9.5 案例研究：Intel Core i7 / Linux 内存系统
9.6 内存映射
9.7 动态内存分配 （重点）
9.8 垃圾收集
